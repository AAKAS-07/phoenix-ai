...
class AdvisoryService:
    ...
    def validate(payload: dict) -> dict:
        """Validate a raw user payload; return a clean numeric feature dict with smart defaults."""
        cleaned = {}
        weather_dict = payload.get('weather') if isinstance(payload.get('weather'), dict) else {}
        defaults = {
            'N': 80.0,
            'P': 40.0,
            'K': 40.0,
            'temperature': float(weather_dict.get('temperature', payload.get('temp', 25.0)) or 25.0),
            'humidity': float(weather_dict.get('humidity', 65.0) or 65.0),
            'ph': float(payload.get('soilPH', payload.get('ph', 6.5)) or 6.5),
            'rainfall': float(weather_dict.get('rainfall', payload.get('rain', 120.0)) or 120.0)
        }
        for feat in FEATURES:
            raw = payload.get(feat)
            if raw is None or (isinstance(raw, str) and raw.strip() == ""):
                value = defaults.get(feat, 50.0)
            else:
                try:
                    value = float(raw)
